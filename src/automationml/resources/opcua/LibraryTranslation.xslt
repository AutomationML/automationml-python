<?xml version="1.0" encoding="UTF-8"?>
<!--TODO: Change xsi:noNamespaceSchemaLocation="CAEX_ClassModel_V2.15.xsd" schema here to CAEX V3-->
<xsl:stylesheet version="2.0" 
	xmlns:fn2="http://whatever"

	xmlns:fn="http://www.w3.org/2005/xpath-functions" 
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	xmlns="http://opcfoundation.org/UA/2011/03/UANodeSet.xsd" 
	xmlns:uax="http://opcfoundation.org/UA/2008/02/Types.xsd"
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
	exclude-result-prefixes="#default xsi xsl exslt fn" 
	xmlns:caex="http://www.dke.de/CAEX" 
	xmlns:exslt="http://exslt.org/common">
	
	<xsl:output method="xml" version="1.0" encoding="UTF-8" indent="yes" omit-xml-declaration="yes"/>

	
	<xsl:template name="ClassReferences">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:variable name="NsName">
			<xsl:call-template name="GetNamespace"/>
		</xsl:variable>
		<xsl:variable name="LibId">
			<xsl:for-each select="ancestor::*[local-name()='InterfaceClassLib' or local-name()='RoleClassLib' or local-name()='SystemUnitClassLib'  or local-name()='AttributeTypeLib']">
				<xsl:value-of select="fn2:remove-space(local-name(.))"/>
			</xsl:for-each>
		</xsl:variable>
		<xsl:comment>
			<xsl:value-of select="concat('Library: ', $NsName)"/>
		</xsl:comment>
		<References>
			<!-- Class hierarchy -->
			<xsl:comment>Hierarchical parent in AutomationML library</xsl:comment>			
			<Reference ReferenceType="Organizes" IsForward="false">
				<xsl:choose>
					<xsl:when test="ancestor::*[fn:local-name()='InterfaceClass'][1]">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', ancestor::*[fn:local-name()='InterfaceClass'][1]/@Name)"/>
					</xsl:when>
					<xsl:when test="ancestor::*[fn:local-name()='RoleClass'][1]">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', ancestor::*[fn:local-name()='RoleClass'][1]/@Name)"/>
					</xsl:when>
					<xsl:when test="ancestor::*[fn:local-name()='SystemUnitClass'][1]">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', ancestor::*[fn:local-name()='SystemUnitClass'][1]/@Name)"/>
					</xsl:when>
					<xsl:when test="ancestor::*[fn:local-name()='AttributeType'][1]">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', ancestor::*[fn:local-name()='AttributeType'][1]/@Name)"/>
					</xsl:when>		
					<xsl:otherwise>
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId)"/>
					</xsl:otherwise>			
				</xsl:choose>			
			</Reference>
			<!-- Forward organization keeps nested AML class trees directly browsable. -->
			<xsl:for-each select="*[local-name()=local-name(current())]">
				<Reference ReferenceType="Organizes">
					<xsl:value-of select="concat('ns=', $NsId, ';s=', fn2:remove-space(@Name))"/>
				</Reference>
			</xsl:for-each>
			
			<xsl:variable name="RawBasePath">
				<xsl:choose>
					<xsl:when test="@RefBaseClassPath"><xsl:value-of select="@RefBaseClassPath"/></xsl:when>
					<xsl:when test="@RefAttributeType"><xsl:value-of select="@RefAttributeType"/></xsl:when>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="BasePath"
				select="if (contains($RawBasePath, '@')) then substring-after($RawBasePath, '@') else string($RawBasePath)"/>
			<xsl:variable name="BaseClass">
				<xsl:call-template name="GetClass">
					<xsl:with-param name="path" select="$BasePath"/>
				</xsl:call-template>
			</xsl:variable>

			<xsl:comment>Parent class</xsl:comment>	
			<Reference ReferenceType="HasSubtype" IsForward="false">
				<xsl:choose>
					<xsl:when test="$BasePath!='' and not(fn:contains($BasePath, '/'))">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $BasePath)"/>
					</xsl:when>
					<xsl:when test="$BasePath!=''">
						<xsl:variable name="ParentNS">
							<xsl:call-template name="GetNamespaceIdByName">
								<xsl:with-param name="Namespace">
									<xsl:value-of select="substring-before($BasePath,'/')"/>
								</xsl:with-param>
							</xsl:call-template>
						</xsl:variable>
						<xsl:variable name="ParentName"
							select="if (normalize-space(string(exslt:node-set($BaseClass)/*/@Name)) != '') then string(exslt:node-set($BaseClass)/*/@Name) else tokenize($BasePath, '/')[last()]"/>
						<xsl:value-of select="concat('ns=', $ParentNS, ';s=', fn2:remove-space($ParentName))"/>
					</xsl:when>
					<xsl:when test="local-name()='SystemUnitClass' and @Name!='AutomationMLBaseSystemUnit'">CAEXObjectType</xsl:when>
					<xsl:when test="local-name()='RoleClass' and @Name!='AutomationMLBaseRole'">AutomationMLBaseRoleClassLib/AutomationMLBaseRole</xsl:when>
					<xsl:when test="local-name()='InterfaceClass' and @Name!='AutomationMLBaseInterface'">AutomationMLInterfaceClassLib/AutomationMLBaseInterface</xsl:when>
					<!--xsl:when test="local-name()='AttributeType' and *[fn:local-name()='Constraint']/*[fn:local-name()='NominalScaledType'][1]">NominalScaledConstraint</xsl:when>
					<xsl:when test="local-name()='AttributeType' and *[fn:local-name()='NominalScaledType']/*[fn:local-name()='OrdinalScaledType']">OrdinalScaledConstraint</xsl:when>
					<xsl:when test="local-name()='AttributeType' and *[fn:local-name()='Constraint']!='' ">UnknownConstraint</xsl:when-->
					<xsl:when test="local-name()='AttributeType'">AMLBaseVariableType</xsl:when>
					<xsl:otherwise>
						<xsl:text>CAEXObjectType</xsl:text>
					</xsl:otherwise>
				</xsl:choose>
			</Reference>
			<xsl:variable name="ObjectId">
				<xsl:call-template name="GetObjectName">
					<xsl:with-param name="Object" select="."/>
				</xsl:call-template>		
			</xsl:variable>
			<xsl:call-template name="References">
				<xsl:with-param name="ObjectId" select="$ObjectId"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
			</xsl:call-template>
		</References>
	</xsl:template>
	<xsl:template name="Library">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:variable name="LibId">
			<!--xsl:text>Library</xsl:text-->
			<xsl:value-of select="fn2:remove-space(local-name(.))"/>
		</xsl:variable>
		<xsl:comment>
			Libraries:
			===============================================
			TODOs:
				- is there a maximum length for NodeId?
				- how do we handle very long lib names -> generate short name?
		</xsl:comment>
		<UAObject>
			<xsl:attribute name="NodeId">
				<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId)"/>
			</xsl:attribute>
			<!--xsl:attribute name="Name">
				<xsl:value-of select="@Name"/>
			</xsl:attribute-->
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="@Name"/>
			</xsl:attribute>
			<DisplayName>
				<xsl:value-of select="@Name"/>
			</DisplayName>
			<xsl:if test="*[fn:local-name()='Description']">
				<Documentation>
					<xsl:value-of select="*[fn:local-name()='Description'][1]"/>
				</Documentation>
			</xsl:if>
			<References>
				<xsl:if test="@ID">
					<Reference ReferenceType="HasProperty">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId, '_AML_ID')"/>
					</Reference>
					<Reference ReferenceType="HasProperty">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId, '__AMLMeta_OriginalID')"/>
					</Reference>
				</xsl:if>
				<xsl:if test="@ChangeMode">
					<Reference ReferenceType="HasProperty">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId, '__AMLMeta_ChangeMode')"/>
					</Reference>
				</xsl:if>
				<xsl:comment>Id no longer part of the specification</xsl:comment>
				<xsl:if test="*[fn:local-name()='Version']">
					<Reference ReferenceType="HasProperty">
						<xsl:value-of select="concat('ns=', $NsId, ';s=', $LibId, '_Version')"/>
					</Reference>
				</xsl:if>
				<Reference ReferenceType="HasTypeDefinition">i=61</Reference>
				<xsl:choose>
					<xsl:when test="local-name(.)='InterfaceClassLib'">
						<xsl:comment>Collection of InterfaceClassLib</xsl:comment>
						<Reference ReferenceType="Organizes" IsForward="false">CAEXFile_InterfaceClassLibs</Reference>
						<!--Reference ReferenceType="HasComponent" IsForward="false">ns=2;s=CAEXFile_InterfaceClassLibs</Reference-->
					</xsl:when>
					<xsl:when test="local-name(.)='RoleClassLib'">
						<xsl:comment>Collection of RoleClassLib</xsl:comment>
						<Reference ReferenceType="Organizes" IsForward="false">CAEXFile_RoleClassLibs</Reference>
						<!--Reference ReferenceType="HasComponent" IsForward="false">ns=2;s=CAEXFile_RoleClassLibs</Reference-->
					</xsl:when>
					<xsl:when test="local-name(.)='SystemUnitClassLib'">
						<xsl:comment>Collection of SystemUnitClassLib</xsl:comment>
						<Reference ReferenceType="Organizes" IsForward="false">CAEXFile_SystemUnitClassLibs</Reference>
						<!--Reference ReferenceType="HasComponent" IsForward="false">ns=2;s=CAEXFile_SystemUnitClassLibs</Reference-->
					</xsl:when>
					<xsl:when test="local-name(.)='AttributeTypeLib'">
						<xsl:comment>Collection of AttributeTypeLib</xsl:comment>
						<!--Reference ReferenceType="Organizes" IsForward="false">ns=1;i=?</Reference-->
						<Reference ReferenceType="Organizes" IsForward="false">CAEXFile_AttributeTypeLibs</Reference>
					</xsl:when>
				</xsl:choose>
				<xsl:comment>TODO: what is the correct way to list all elements of the Lib?</xsl:comment>
				<xsl:comment><xsl:value-of select="concat('List of included ', fn:replace(local-name(), 'Lib', ''))"/></xsl:comment>
				<xsl:for-each select="./*[local-name()='RoleClass' or local-name()='SystemUnitClass' or local-name()='InterfaceClass' or local-name()='AttributeType']">
					<Reference ReferenceType="Organizes"><xsl:value-of select="concat('ns=',$NsId, ';s=',fn2:remove-space(@Name))"/></Reference>
				</xsl:for-each>
			</References>
		</UAObject>
		<xsl:if test="@ID">
			<xsl:call-template name="StringAttributeVariable">
				<xsl:with-param name="ObjectName" select="@Name"/>
				<xsl:with-param name="ParentId" select="$LibId"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="AttributeName" select="'AML_ID'"/>
				<xsl:with-param name="AttributeValue" select="if (starts-with(@ID, '{')) then string(@ID) else concat('{', @ID, '}')"/>
			</xsl:call-template>
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="$LibId"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="'OriginalID'"/>
				<xsl:with-param name="MetadataValue" select="string(@ID)"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="@ChangeMode">
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="$LibId"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="'ChangeMode'"/>
				<xsl:with-param name="MetadataValue" select="string(@ChangeMode)"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	<!-- .........................................................................
		InterfaceClassLib: Create UAObjectTypes
	.........................................................................-->
	<xsl:template match="*[fn:local-name()='InterfaceClassLib']">
		<xsl:comment>
			InterfaceClassLib: <xsl:value-of select="@Name"/>
			==================================================
			TODOs:
				- How to handle DefaultValues for attributes?
				- What is the corresponding UAObjectType for AutomationMLBaseInterface?
				- How to handle if the parent interface is not specified in the InterfaceClassLibs of the document?
				- compare to line 79ff. https://github.com/OPCFoundation/UA-Nodeset/blob/latest/AML/Opc.Ua.AMLLibraries.NodeSet2.xml
					- why is there in InternalLink (ns=1;i=4002) to AutomationMLBaseInterface (ns=2;i=22)?
					- why is there an additional ID attribute (ns=2;i=2)?
		</xsl:comment>
		<xsl:call-template name="Library"/>
		<xsl:apply-templates select="*"/>
	</xsl:template>
	<xsl:template match="//*[fn:local-name()='InterfaceClass']">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:comment><xsl:value-of select="concat('InterfaceClass: ', @Name)"/></xsl:comment>
		<UAObjectType>
			<xsl:attribute name="NodeId">
				<xsl:value-of select="concat('ns=', $NsId, ';s=', fn2:remove-space(@Name))"/>
			</xsl:attribute>
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="@Name"/>
			</xsl:attribute>
			<DisplayName>
				<xsl:value-of select="@Name"/>
			</DisplayName>
			<xsl:if test="*[local-name()='Description']">
				<Documentation>
					<xsl:value-of select="*[local-name()='Description'][1]"/>
				</Documentation>
			</xsl:if>
			<xsl:call-template name="ClassReferences"/>
		</UAObjectType>
		<xsl:apply-templates select="node()|@*"/>
	</xsl:template>
	<!-- .........................................................................
		RoleClassLib: Create UAObjectTypes
	.........................................................................-->
	<xsl:template match="*[fn:local-name()='RoleClassLib']">
		<xsl:comment>
			RoleClassLib: <xsl:value-of select="@Name"/>
			==================================================
			TODOs:
				- What was the correct translation for an RoleClass?
				- Why is the lib modelled as UAObject?
		</xsl:comment>
		<xsl:call-template name="Library"/>
		<xsl:apply-templates select="node()"/>
	</xsl:template>
	
	<xsl:template match="//*[fn:local-name()='RoleClass']">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:comment><xsl:value-of select="concat('RoleClass: ', @Name)"/></xsl:comment>
		<UAObjectType>
			<xsl:attribute name="NodeId">
				<xsl:value-of select="concat('ns=', $NsId, ';s=', fn2:remove-space(@Name))"/>
			</xsl:attribute>
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="@Name"/>
			</xsl:attribute>
			<DisplayName>
				<xsl:value-of select="@Name"/>
			</DisplayName>
			<xsl:if test="*[local-name()='Description']">
				<Documentation>
					<xsl:value-of select="*[local-name()='Description'][1]"/>
				</Documentation>
			</xsl:if>
			<xsl:call-template name="ClassReferences"/>
		</UAObjectType>
		<xsl:apply-templates select="node()|@*"/>
	</xsl:template>
	<!-- .........................................................................
		SystemUnitClassLib: Create UAObjectTypes
	.........................................................................-->
	<xsl:template match="*[fn:local-name()='SystemUnitClassLib']">
		<xsl:comment>
			SystemUnitClassLib: <xsl:value-of select="@Name"/>
			==================================================
			TODOs:
				- What was the correct translation for a SystemUnitClass?
				- Why is the lib modelled as UAObject?
		</xsl:comment>
		<xsl:call-template name="Library"/>
		<xsl:apply-templates select="node()"/>
	</xsl:template>
	<xsl:template match="//*[fn:local-name()='SystemUnitClass']">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:variable name="ObjectName">
			<xsl:call-template name="GetObjectName">
				<xsl:with-param name="Object" select="."/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="ObjectId">
			<xsl:call-template name="FormatRef">
				<xsl:with-param name="ObjectId" select="@ID"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:comment><xsl:value-of select="concat('SystemUnitClass: ', @Name)"/></xsl:comment>
		<UAObjectType>
			<xsl:attribute name="NodeId">
				<xsl:value-of select="concat('ns=', $NsId, ';s=', fn2:remove-space(@Name))"/>
			</xsl:attribute>
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="@Name"/>
			</xsl:attribute>
			<DisplayName>
				<xsl:value-of select="@Name"/>
			</DisplayName>
			<xsl:if test="*[local-name()='Description']">
				<Documentation>
					<xsl:value-of select="*[local-name()='Description'][1]"/>
				</Documentation>
			</xsl:if>
			<xsl:call-template name="ClassReferences"/>
		</UAObjectType>
		<xsl:apply-templates select="node()|@*"/>
	</xsl:template>
	<!-- .........................................................................
		AttributeTypeLib: Create UAObjectTypes
	.........................................................................-->
	<xsl:template match="*[fn:local-name()='AttributeTypeLib']">
		<xsl:comment>
			AttributeTypeLib: <xsl:value-of select="@Name"/>
			==================================================
			TODOs:
				- What was the correct translation for a AttributeType?
		</xsl:comment>
		<xsl:call-template name="Library"/>
		<xsl:apply-templates select="node()"/>
	</xsl:template>
	<xsl:template match="//*[fn:local-name()='AttributeType']">
		<xsl:variable name="NsId">
			<xsl:call-template name="GetNamespaceId"/>
		</xsl:variable>
		<xsl:variable name="AttributeDataType" select="lower-case(normalize-space(string(@AttributeDataType)))"/>
		<xsl:variable name="dtList">
			<xsl:copy-of select="exslt:node-set($DataTypes)/*[lower-case(normalize-space(string(@AML)))=$AttributeDataType]"/>
		</xsl:variable>
		<xsl:variable name="Datatype">
			<xsl:choose>
				<xsl:when test="$dtList/*/@OPCAlias!=''">
					<xsl:value-of select="$dtList/*[1]/@OPCAlias"/>
				</xsl:when>
				<xsl:when test="$dtList/*/@OPCNodeId!=''">
					<xsl:value-of select="$dtList/*[1]/@OPCNodeId"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="normalize-space(string(@AttributeDataType))"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:comment><xsl:value-of select="concat('AttributeType: ', @Name)"/></xsl:comment>
		<UAVariableType>
			<xsl:attribute name="NodeId">
				<xsl:value-of select="concat('ns=', $NsId, ';s=', fn2:remove-space(@Name))"/>
			</xsl:attribute>
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="@Name"/>
			</xsl:attribute>
			<xsl:if test="$Datatype!=''">
				<xsl:attribute name="DataType"><xsl:value-of select="$Datatype"/></xsl:attribute>
			</xsl:if>
			<DisplayName>
				<xsl:value-of select="@Name"/>
			</DisplayName>
			<xsl:if test="*[local-name()='Description']">
				<Documentation>
					<xsl:value-of select="*[local-name()='Description'][1]"/>
				</Documentation>
			</xsl:if>
			<xsl:call-template name="ClassReferences"/>
			<xsl:if test="*[local-name()='Value']">
				<Value>
					<xsl:choose>
						<xsl:when test="$Datatype='String' or $Datatype='Boolean' or contains($Datatype, 'Int')">
							<xsl:element name="{concat('uax:', $Datatype)}"><xsl:value-of select="*[local-name()='Value'][1]"/></xsl:element>
						</xsl:when>
						<xsl:otherwise><uax:String><xsl:value-of select="*[local-name()='Value'][1]"/></uax:String></xsl:otherwise>
					</xsl:choose>
				</Value>
			</xsl:if>
		</UAVariableType>
		<xsl:if test="@AttributeDataType">
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="@Name"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="'AttributeDataType'"/>
				<xsl:with-param name="MetadataValue" select="string(@AttributeDataType)"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="*[local-name()='DefaultValue']">
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="@Name"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="'DefaultValue'"/>
				<xsl:with-param name="MetadataValue" select="string(*[local-name()='DefaultValue'][1])"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:if test="@Unit">
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="@Name"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="'Unit'"/>
				<xsl:with-param name="MetadataValue" select="string(@Unit)"/>
			</xsl:call-template>
		</xsl:if>
		<xsl:for-each select="*[local-name()='RefSemantic']">
			<xsl:call-template name="AMLAttributeMetadataVariable">
				<xsl:with-param name="ParentObjectId" select="../@Name"/>
				<xsl:with-param name="Namespace" select="$NsId"/>
				<xsl:with-param name="MetadataName" select="concat('RefSemantic_', position())"/>
				<xsl:with-param name="MetadataValue" select="string(@CorrespondingAttributePath)"/>
			</xsl:call-template>
		</xsl:for-each>
		<xsl:apply-templates select="node()|@*"/>
	</xsl:template>
</xsl:stylesheet>
